    ...
    createRouter(location) {
      if (this.exportAsTypescript === "on") {
        fs.writeFileSync(
          ...
          this.createRouterImports(this.routes) + this.createExport(this.routes)
        );
      } else {
        fs.writeFileSync(
...
test('renders ${componentName}', () => {
  ...
  // customize your tests here; for more info please visit: https://github.com/vuejs/test-utils/
    ...
    createComponentTestCode(componentLocation, componentName, componentMap) {
      fs.writeFileSync(
        componentLocation,
        this.writeRenderUnitTestString(
          componentName,
   ...
   <el-badge :value="2" class="item" type="warning">
    ...
    writeTemplate(componentName, children, routes) {
      ...
      if (this.componentMap[componentName].htmlAttributes) {
        ...
        let compClass = this.routes.hasOwnProperty(componentName)
          ? componentName
...
.router-view {
  ...
  width: 1280px;
    ...
    createOauthFile(location) {
      if (
        this.$store.state.exportOauth === "on" ||
        this.$store.state.exportOauthGithub === "on"
      ) {
        let str = `<template>`;
    ...
    createStore(location) {
      ...
      for (let i = 0; i < this.userActions.length; i++) {
        ...
        str += `\n\t\t},`;
        if (i === this.userActions.length - 1) {
          str = str.slice(0, -1);
        }
      }
      str += `\n\t},`;
