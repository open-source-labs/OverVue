...
<!-- <script setup>
    ...
    export default function redoMixin () {
    const doneAction = [];
    const undoneAction = [];
  ...
  const store = useStore();
      ...
      const subscribeAction = store.subscribeAction((action, state) => {
          if (typeof action.payload === "object") {
            action.payload = cloneDeep(action.payload);
          }
          doneAction.push(action);
          if (!isTimetraveling) {
            if (undoneAction[undoneAction.length - 1]) {
              if (
                action.type ===
                undoneAction[undoneAction.length - 1].type &&
                deepEqual(
                  action.payload,
                  undoneAction[undoneAction.length - 1].payload
                )
              ) {
                undoneAction.pop();
              } else {
      ...
      const undo = () => {
          if (doneAction.length === 0) {
            return;
          }
    
          isTimetraveling = true;
          let undone = doneAction.pop();
          ...
          if (undone !== undefined) {
            undoneAction.push(undone);
            if (ignoredActions.has(undone.type)) {
              while (
                doneAction[doneAction.length - 1] &&
                ignoredActions.has(doneAction[doneAction.length - 1].type)
              ) {
                undoneAction.push(doneAction.pop());
              }
              ...
              let finalPop = doneAction.pop();
              if (finalPop !== undefined) {
                undoneAction.push(finalPop);
              }
            }
          }
          ...
          $store.commit("EMPTY_STATE", payload);
          doneAction.forEach((action) => {
            $store.dispatch(action.type, cloneDeep(action.payload));
            doneAction.pop();
          });
